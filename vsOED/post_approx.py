...
#                 sigma_layers.append(nn.BatchNorm1d(sigma_dimns[i + 1]))
    ...
    def forward(self, inputs, thetas):
        ...
        if len(self.idxs_norm) > 0:
            normal = torch.distributions.Normal(mus[..., idxs_norm], sigmas[..., idxs_norm])
            logprob_mixture_normal = normal.log_prob(thetas[..., idxs_norm])
        else:
            logprob_mixture_normal = 0

        if len(self.idxs_truncnorm) > 0:
            truncnorm = TruncatedNormal(mus[..., idxs_truncnorm], sigmas[..., idxs_truncnorm], 
                self.truncnorm_bounds[:, 0], self.truncnorm_bounds[:, 1])
            logprob_mixture_truncnorm = truncnorm.log_prob(thetas[..., idxs_truncnorm])
        else:
        ...
        probs = (prob_mixture * weights).sum(-1) # (N)
...
class POST_APPROX(object):
        ...
        if self.model_weight != 0:
            ...
            for i, stage in enumerate(stages_incre):
                if stage == stages_incre[-1] or not share_interm_net:
                    ...
                    model_post_dimns = [(stages_incre[-2] + 1) * (self.n_design + self.n_obs), 256, 256, 256, self.n_model]
                model_post_layers = []
                for i in range(len(model_post_dimns) - 1):
                    model_post_layers.append(nn.Linear(model_post_dimns[i], model_post_dimns[i + 1]))
        #             feature_layers.append(nn.BatchNorm1d(feature_dimns[i + 1]))
            ...
            for i, stage in enumerate(stages_incre):
                if stage == stages_incre[0] or stage == stages_incre[-1] or not share_interm_net:
                    ...
                    for j, n_poi in enumerate(self.n_pois):
                        ...
                        if not self.use_NFs:
                            post_net = GMM_NET(h * (self.n_design + self.n_obs), n_poi, n_mixture, mu_bounds['poi'][j], max_sigmas['poi'][j], truncnorm_info['poi'][j], activate)
                        else:
                            ###########################################
                            post_net = NFs()
                            ###########################################
            ...
            for i, stage in enumerate(stages_incre):
                if stage == stages_incre[0] or stage == stages_incre[-1] or not share_interm_net:
                    ...
                    for j, n_goal in enumerate(self.n_goals):
                        h = stage + 1 if stage == stages_incre[-1] or not share_interm_net else stages_incre[-2] + 1
                        if not self.use_NFs:
                            post_net = GMM_NET(h * (self.n_design + self.n_obs), n_goal, n_mixture, mu_bounds['goal'][j], max_sigmas['goal'][j], truncnorm_info['goal'][j], activate)
                        else:
                            ###########################################
                            post_net = NFs()
                            ###########################################
    ...
    def form_input(self, stage, ds_hist, ys_hist):
        if stage >= self.stages_incre[-1] or not self.share_interm_net:
            ...
            X0 = torch.zeros(len(ds_hist), len(self.stages_incre) - 1)
            X0[:, idx] = 1.0
        ...
        for stage in self.stages_incre:
            ...
            if self.goal_weight != 0:
                for scheduler in self.goal_post_schedulers[stage]:
                    scheduler.step()
            if l % self.log_every == 0 and self.dowel is not None and stage == self.stages_incre[-1]:
                if self.model_weight != 0:

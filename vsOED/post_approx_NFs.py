...
class cINN(nn.Module):  
    def __init__(self, n_theta, n_input, nodes_s, nodes_t, act, network=FCNN, alpha = None):
        super().__init__()
    ...
    def forward(self, inputs, thetas):             #    def forward(self, theta, y):   
...
class NFs(nn.Module):
    def __init__(self, network, n_input, n_theta, num_trans):
        super().__init__()
        #self.device     = device
        self.n_theta    = n_theta
        self.flows      = [cINN(n_theta, n_input, [256, 256, 256], [256, 256, 256], nn.ReLU(), network, alpha = 20) for i in range(num_trans)]

        self.flows_list   = nn.ModuleList(self.flows) 
        self.nb_flows     = len(self.flows)
        self.feature_net  = network(n_input, n_input, [256, 256], nn.ReLU())    #
    ...
    #    index   = (torch.abs(thetas) < 10).all(axis = 1).cpu()
       # norm_logprob  =  torch.distributions.MultivariateNormal(torch.zeros(thetas.shape[1]).to(self.__dir__device), 
      #                      torch.eye(thetas.shape[1]).to(self.device)).log_prob(thetas)
...
class POST_APPROX_NF(object):
    def __init__(self,  n_stage, n_design, n_obs, 
        ...
        n_mixture=8, n_trans=4, activate=None, prior=None,       ###########
        n_incre=1, share_interm_net=False, network = FCNN,       #
                ...
                if stage == stages_incre[0] or stage == stages_incre[-1] or not share_interm_net:
                    ...
                    for j, n_poi in enumerate(self.n_pois):
                        ...
                        if not self.use_NFs:
                            ...
                            ##############################################
                            post_net = NFs( self.network, input_dimn, n_poi,  self.num_trans)
                            ##############################################

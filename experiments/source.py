...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Source location problem")
    parser.add_argument("--id", default=0, type=int) # From 0 to 9
    parser.add_argument("--n-stage", default=30, type=int) # From 1 to 30
    parser.add_argument("--model", default="uni", type=str) # uni or multi
    parser.add_argument("--include-nuisance", default=False, type=bool) # True or False
    parser.add_argument("--include-goal", default=False, type=bool) # True or False
    ...
    if args.model == 'uni':
        n_pois = [4]
        n_nuisps = [2] if args.include_nuisance else None
        n_goals = [1] if args.include_goal else None
    elif args.model == 'multi':
        n_pois = [2, 4, 6]
        n_nuisps = [2, 2, 2] if args.include_nuisance else None
        n_goals = [1, 1, 1] if args.include_goal else None
    ...
    model = SOURCE(multimodel=(args.model=='multi'), include_nuisance=args.include_nuisance, include_goal=args.include_goal)
    prior = model
    ...
    try:
        ...
        pass
    dowel.logger.push_prefix(f'[SOURCE-{n_stage}stage] ')
    ...
    vsoed.asses(2000, n_contrastive_sample=int(1e6), return_all=True, dowel=dowel, save_path=save_folder + '/evaluation.pt')
    if not use_NFs:
        set_random_seed(EVAL_SEEDS[id])
        dowel.logger.log('Evaluating using post approx...')
        vsoed.asses(int(1e6), use_PCE=False, return_all=True, dowel=dowel, save_path=save_folder + '/evaluation_with_post_approx.pt')